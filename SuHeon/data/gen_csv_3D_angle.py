...
while video.isOpened():
    ...
    if results.pose_world_landmarks is not None and all(results.pose_world_landmarks.landmark[j].visibility > 0.1 for j in [11, 12, 23, 24, 25, 26, 27, 28]):
        ...
        # 왼쪽 허리 각도 계산 및 저장
        back_angle_left = round(modules.calculateAngle3D(landmarks_adjust_ratio[mp_pose.PoseLandmark.LEFT_SHOULDER.value],
        ...
        # 오른쪽 허리 각도 계산 및 저장
        back_angle_right = round(modules.calculateAngle3D(landmarks_adjust_ratio[mp_pose.PoseLandmark.RIGHT_SHOULDER.value],
        ...
        # 왼쪽 무릎 각도 계산 및 저장
        knee_angle_left = round(modules.calculateAngle3D(landmarks_adjust_ratio[mp_pose.PoseLandmark.LEFT_ANKLE.value],
                                               landmarks_adjust_ratio[mp_pose.PoseLandmark.LEFT_KNEE.value],
                                               landmarks_adjust_ratio[mp_pose.PoseLandmark.LEFT_HIP.value]), 1)

        # 오른쪽 무릎 각도 계산 및 저장
        knee_angle_right = round(modules.calculateAngle3D(landmarks_adjust_ratio[mp_pose.PoseLandmark.RIGHT_ANKLE.value],
        ...
        ankle_knee_knee_left = round(modules.calculateAngle3D(landmarks_adjust_ratio[mp_pose.PoseLandmark.LEFT_ANKLE.value],
                                                    landmarks_adjust_ratio[mp_pose.PoseLandmark.LEFT_KNEE.value],
                                                    landmarks_adjust_ratio[mp_pose.PoseLandmark.RIGHT_KNEE.value]), 1)

        # 발목-무릎-반대쪽 무릎 오른쪽 각도 계산 및 저장
        ankle_knee_knee_right = round(modules.calculateAngle3D(landmarks_adjust_ratio[mp_pose.PoseLandmark.RIGHT_ANKLE.value],
                                                     landmarks_adjust_ratio[mp_pose.PoseLandmark.RIGHT_KNEE.value],
                                                     landmarks_adjust_ratio[mp_pose.PoseLandmark.LEFT_KNEE.value]), 1)

        # 무릎-엉덩이-반대쪽엉덩이 왼쪽 각도 계산 및 저장
        hip_hip_knee_left = round(modules.calculateAngle3D(landmarks_adjust_ratio[mp_pose.PoseLandmark.LEFT_KNEE.value],
                                                 landmarks_adjust_ratio[mp_pose.PoseLandmark.LEFT_HIP.value],
                                                 landmarks_adjust_ratio[mp_pose.PoseLandmark.RIGHT_HIP.value]), 1)

        # 무릎-엉덩이-반대쪽엉덩이 오른쪽 각도 계산 및 저장
        hip_hip_knee_right = round(modules.calculateAngle3D(landmarks_adjust_ratio[mp_pose.PoseLandmark.RIGHT_KNEE.value],
                                                  landmarks_adjust_ratio[mp_pose.PoseLandmark.RIGHT_HIP.value],
                                                  landmarks_adjust_ratio[mp_pose.PoseLandmark.LEFT_HIP.value]), 1)

        # 무릎-무릎 사이거리 계산 및 저장
        knee_knee_dis = round(modules.calculateDistance3D(landmarks_adjust_ratio[mp_pose.PoseLandmark.RIGHT_KNEE.value],
                                                          landmarks_adjust_ratio[mp_pose.PoseLandmark.LEFT_KNEE.value]), 1)
