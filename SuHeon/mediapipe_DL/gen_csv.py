...
video = cv2.VideoCapture('./my_squat_test.mp4')

# 발 사이 거리, 허리각도RL, 무릎각도RL, 엉덩이에서 뒤꿈치거리RL
inter_foots = []
back_angle_R = []
back_angle_L = []
knee_angle_R = []
...
while video.isOpened():
    ...
    # 두 뒷발꿈치 사이 거리 계산 및 저장
    inter_foots_value = calculateDistance(landmarks[mp_pose.PoseLandmark.LEFT_HEEL.value],
                                          landmarks[mp_pose.PoseLandmark.RIGHT_HEEL.value])
    inter_foots.append(inter_foots_value)
    ...
    # 왼쪽 엉덩이, 뒷발꿈치 사이 거리 계산 및 저장
    hip_to_heel_left = calculateDistance(landmarks[mp_pose.PoseLandmark.LEFT_HIP.value],
                                         landmarks[mp_pose.PoseLandmark.LEFT_HEEL.value])
    hip_to_heel_L.append(hip_to_heel_left)

    # 오른쪽 엉덩이, 뒷발꿈치 사이 거리 계산 및 저장
    hip_to_heel_right = calculateDistance(landmarks[mp_pose.PoseLandmark.RIGHT_HIP.value],
                                          landmarks[mp_pose.PoseLandmark.RIGHT_HEEL.value])
    hip_to_heel_R.append(hip_to_heel_right)
...
# 데이터프레임 만들기
df = pd.DataFrame(inter_foots, columns = ['inter_foots'])
df['back_angle_R'] = back_angle_R
df['back_angle_L'] = back_angle_L
df['knee_angle_R'] = knee_angle_R
df['knee_angle_L'] = knee_angle_L
df['hip_to_heel_R'] = hip_to_heel_R
df['hip_to_heel_L'] = hip_to_heel_L
