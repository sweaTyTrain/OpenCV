...
def isPushUp(left_elbow_angle, pushUpCnt, pushUpBeforeState, pushUpNowState):
    # 만약 오른쪽 무릎의 각도가 140 이상이면 0(서있는 상태)
    if left_elbow_angle >= 140:
        pushUpNowState = 0
    # 90 이하라면 1(앉은 상태)
    elif left_elbow_angle <= 90 and left_elbow_angle > 0:
        pushUpNowState = 1

    # 만약 squatBeforeState(이전 상태)가 1(앉은 상태)였는데
    # squatNowState(현재 상태)가 0(서있는 상태)가 되면 squatCnt증가
    if pushUpBeforeState == 1 and pushUpNowState == 0:
        pushUpCnt += 1

    # 다음 프레임을 받아오기 전에 squatNowState를 squatBeforeState에 기억시킨다.
    pushUpBeforeState = pushUpNowState

    return left_elbow_angle, pushUpCnt, pushUpBeforeState, pushUpNowState
...
while video.isOpened():
    ...
    if isLandmarkDetected(results, frame):
        ...
        # 왼쪽 팔꿈치각 계산
        left_elbow_angle = calculateAngle(mp_pose.PoseLandmark.LEFT_SHOULDER.value,
                                          mp_pose.PoseLandmark.LEFT_ELBOW.value,
                                          mp_pose.PoseLandmark.LEFT_WRIST.value, min_visibility, results, landmarks)
        ...
        if left_elbow_angle == -1:
            ...
            # 스쿼트 인식 여부
            left_elbow_angle, pushUpCnt, pushUpBeforeState, pushUpNowState = isPushUp(left_elbow_angle, pushUpCnt, pushUpBeforeState, pushUpNowState)
    else:
        cv2.putText(frame, "all landmarks not detected", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
        print("랜드마크 검출 불가")

    cv2.putText(frame, f"pushUpCnt: {pushUpCnt}", (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
    print("푸쉬 업 개수:", pushUpCnt)
