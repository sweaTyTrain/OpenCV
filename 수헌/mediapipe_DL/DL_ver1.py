...
for i, rows in data_train.iterrows():
    x_train.append([rows['inter_foots'], rows['back_angle_R'], rows['back_angle_L'], rows['knee_angle_R'], rows['knee_angle_L'], rows['hip_to_heel_R'], rows['hip_to_heel_L']])

# x_test에 입력값으로 넣을 값들 불러오기
for i, rows in data_test.iterrows():
    x_test.append([rows['inter_foots'], rows['back_angle_R'], rows['back_angle_L'], rows['knee_angle_R'], rows['knee_angle_L'], rows['hip_to_heel_R'], rows['hip_to_heel_L']])
...
# 모델 저장
# model.save('./models/model_ver1.h5')